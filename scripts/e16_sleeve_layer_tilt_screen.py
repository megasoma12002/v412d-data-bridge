#!/usr/bin/env python3
"""Stage A paper screen: sleeve-layer Soft-Frozen router tilt vs LIVE_STACK.

Charter: research/ops/SLEEVE_LAYER_TILT_CHARTER.md
...
OUT = ROOT / "repro/sleeve-layer-tilt"
...
# Stage A finite grid (≤24 challengers)
ALPHAS = (0.05, 0.10, 0.15, 0.20)
# All Stage A signals are oversold / below-trend → sign +1
SIGNALS = (
    ("BELOW_MA60", "SLEEVE_BELOW_MA60", 60, "ma"),
    ("BELOW_MA40", "SLEEVE_BELOW_MA40", 40, "ma"),
    ("BELOW_MA120", "SLEEVE_BELOW_MA120", 120, "ma"),
    ("RSI14_LT30", "SLEEVE_RSI14_LT30", 14, "rsi_lt30"),
    ("MOM20_NEG", "SLEEVE_MOM20_NEG", 20, "mom_neg"),
)
...
def main() -> int:
    ...
    jobs = []
    for short, _sig_id, window, kind in SIGNALS:
    ...
    payload = {
        ...
        "label": "SLEEVE_LAYER_TILT_SCREEN_STAGE_A",
        "charter": "research/ops/SLEEVE_LAYER_TILT_CHARTER.md",
    ...
    (OPS / "SLEEVE_LAYER_TILT_SCREEN.json").write_text(
        json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
    )
    ...
    lines = [
        ...
        f"Charter: `SLEEVE_LAYER_TILT_CHARTER.md`",
        ...
        f"- Grid: signals `{[s[0] for s in SIGNALS]}` · α `{list(ALPHAS)}` · sign +1",
        "",
        "## Top 20",
        "",
    ...
    lines += [
        ...
        "## Label",
        "",
        f"`SLEEVE_LAYER_TILT_SCREEN_STAGE_A_2026-09-10__{verdict}`",
        "",
    ]
    md = "\n".join(lines)
    (OPS / "SLEEVE_LAYER_TILT_SCREEN.md").write_text(md, encoding="utf-8")
    (OUT / "reports" / "SLEEVE_LAYER_TILT_SCREEN.md").write_text(md, encoding="utf-8")
    ...
    zh = "\n".join(
        [
            ...
            "",
            "英文：`SLEEVE_LAYER_TILT_SCREEN.md`",
            "",
        ]
    )
    (OPS / "SLEEVE_LAYER_TILT_SCREEN.zh-TW.md").write_text(zh, encoding="utf-8")
