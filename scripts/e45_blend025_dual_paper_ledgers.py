#!/usr/bin/env python3
"""E45 blend-α=0.25 dual-paper ledgers (OPERATING OBSERVE — not live).

Side-by-side Exact T+1 paper books:
  BASE_E16_E18_E22_v2s — Soft-Frozen early-stack + formal E22_v2s
  BLEND_E45_A25       — same stack + 25% intensity E45 E3_VOLTARGET_WINNER overlay

Opened by human ballot: ``E45 OPEN blend-α=0.25 observe``.
Parent screen: ``research/e45/E45_BLEND_ALPHA_PAPER_SCREEN.md``.
...
def main() -> None:
    ...
    e45_blend = ((1.0 - BLEND_ALPHA) * 1.0 + BLEND_ALPHA * e45_full.astype(float)).clip(0.0, 1.0)
    e45_blend = e45_blend.rename("e45_blend_a25_exposure")
    ...
    nav_c.to_csv(OUT / "outputs" / "blend_e45_a25_daily_nav.csv", index=False)
    fills_b.to_csv(OUT / "outputs" / "base_e16_e18_e22_v2s_fills.csv", index=False)
    fills_c.to_csv(OUT / "outputs" / "blend_e45_a25_fills.csv", index=False)
    base_target.to_csv(OUT / "outputs" / "base_targets.csv")
    e45_blend.to_csv(OUT / "outputs" / "blend_e45_a25_exposure.csv")

    jb = nav_b[["date", "nav"]].rename(columns={"nav": "nav_base"})
    jc = nav_c[["date", "nav"]].rename(columns={"nav": "nav_blend_e45_a25"})
    joined = jb.merge(jc, on="date", how="inner")
    joined["rel_blend_vs_base"] = joined["nav_blend_e45_a25"] / joined["nav_base"]
    ...
    proposal = {
        ...
        "ballot": "E45 OPEN blend-α=0.25 observe",
        "base_id": BASE_ID,
        ...
        "ops_checklist": [
            ...
            "Run BASE + BLEND_E45_A25 paper ledgers in parallel with month-end monitor",
    ...
    (OUT / "reports" / "e45_blend025_dual_paper_observe.json").write_text(
        json.dumps(proposal, indent=2) + "\n"
    )
    (RESEARCH / "E45_BLEND025_DUAL_PAPER_OBSERVE.json").write_text(json.dumps(proposal, indent=2) + "\n")
    ...
    lines = [
        ...
        f"- **{CHAL_ID}**: same stack + α=0.25 × E45 `{E45_PROFILE_DEFAULT}` exposure overlay",
    ...
    lines += [
        ...
        "",
        "Artifacts:",
        f"- `{OUT / 'reports' / 'e45_blend025_dual_paper_observe.json'}`",
        f"- `{OUT / 'outputs' / 'dual_paper_nav_compare.csv'}`",
        "",
    ]
    md = "\n".join(lines)
    (OUT / "E45_BLEND025_DUAL_PAPER_OBSERVE.md").write_text(md)
    (RESEARCH / "E45_BLEND025_DUAL_PAPER_OBSERVE.md").write_text(md)
    print(
        json.dumps(
            {
