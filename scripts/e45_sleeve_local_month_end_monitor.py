#!/usr/bin/env python3
"""E45 sleeve-local FIN_ONLY α=0.10 month-end dual-paper monitor (ops observe — not live).

Compares BASE_E16_E18_E22_v2s vs SLEEVE_FIN_ONLY_A10 paper NAVs at month-end (or as-of).
...
Optional: re-run `scripts/e45_sleeve_local_dual_paper_ledgers.py` first to refresh.
"""
...
CHAL_NAV = ROOT / "repro/e45-sleeve-local-dual-paper-observe/outputs/sleeve_fin_only_a10_daily_nav.csv"
COMPARE_NAV = ROOT / "repro/e45-sleeve-local-dual-paper-observe/outputs/dual_paper_nav_compare.csv"
RESEARCH = ROOT / "research/gaps"

LOCKED_ID = "SLEEVE_FIN_ONLY_A10"
# Dynamic trailing gates (YTD / 1y).
TRAIL_ALERT_PP = 3.0
TRAIL_PAUSE_PP = 5.0
# Design-known structural givebacks (Stage-3 design pack, 2026-09-05).
DESIGN_GIVEBACK_PP = {
    # From E45_SLEEVE_LOCAL_DEEP_DIVE FIN_ONLY@α=0.10 (2026-09-06)
    "heldout_2019_plus": 1.25,
    "sealed_2023_plus": 1.80,
}
...
def main() -> None:
    ...
    base, chal, nav_source = load_pair_nav(
        ...
        CHAL_NAV,
        COMPARE_NAV,
        chal_col="nav_sleeve_fin_only_a10",
        refresh_hint="scripts/e45_sleeve_local_dual_paper_ledgers.py",
    )
        ...
        rows.append(
            {
                ...
                "sleeve_fin_only_a10_cagr": sc["cagr"],
                "sleeve_fin_only_a10_mdd": sc["max_drawdown"],
    ...
    summary = {
        ...
        "note": (
            ...
            "E45 sleeve-local FIN_ONLY α=0.10 dual-paper ≠ stitch license. "
    ...
    lines = [
        f"# E45 Blend-α=0.10 FIN_ONLY Month-End Paper Monitor — asof {asof.date()}",
        "",
        f"Generated: `{summary['generated_at_utc']}`",
        "Status: **OPERATING OBSERVE / PAPER ONLY** — Soft-Frozen live default unchanged.",
        f"Locked: **{LOCKED_ID}** (α=0.10 FIN_ONLY × E45 `E3_VOLTARGET_WINNER` on early-stack)",
        "",
        ...
        "",
        "| Window | BASE CAGR | BASE MDD | SLEEVE_FIN_ONLY_A10 CAGR | SLEEVE_FIN_ONLY_A10 MDD | "
    ...
    for r in rows:
        ...
        chal_cagr_note = _pct(r["sleeve_fin_only_a10_cagr"])
        if r["window"] == "mtd":
        ...
        lines.append(
            ...
            f"{chal_cagr_note} | {_pct(r['sleeve_fin_only_a10_mdd'])} | "
    ...
    (RESEARCH / "E45_SLEEVE_LOCAL_MONTH_END_MONITOR.md").write_text(md)
    (RESEARCH / "E45_SLEEVE_LOCAL_MONTH_END_MONITOR.json").write_text(json.dumps(summary, indent=2) + "\n")
    print(
        json.dumps(
            {
                "asof": summary["asof"],
