#!/usr/bin/env python3
"""E22_v3 paper challenger — EXPERIMENTAL cash_payment_date dividend path.

Version: E22_v3_CASH_PAY_PAPER_CHALLENGER
Status: EXPERIMENTAL paper only (H1 sandbox follow-on). NOT SOFT_FROZEN.

Same E16 targets / Exact T+1 / fees as E22_v2, but cash dividend credits on
``cash_payment_date``. Writes only under ``forward/e22_v3_challenger/``.

Does NOT edit ``forward/e22_v2/`` or ``forward/e21/``. Promotion requires
explicit approval of a new E22_v3_* official version.
"""
...
VERSION_ID = "E22_v3_CASH_PAY_PAPER_CHALLENGER"
PACKAGE = "e22_v3_challenger"
DEFAULT_STATE = "forward/e22_v3_challenger"
SEEDED_AT = "2026-09-04"
APPROVAL_PHRASE = (
    "NOT APPROVED — paper challenger only; require explicit APPROVE E22_v3_* "
    "before any official cutover."
)
...
def load_div_map(path: Path) -> dict:
    """Credit map keyed by cash_payment_date (H1 challenger rule)."""
    if not path.exists():
        return {}
    ...
    d = d.dropna(subset=["cash_payment_date", "cash_dividend"])
    d = d[d["code"].isin(ALL)]
    d = d[d["cash_dividend"] > 0]
    out: dict[str, list[tuple[str, float]]] = {}
    for _, row in d.iterrows():
        key = pd.Timestamp(row["cash_payment_date"]).date().isoformat()
        out.setdefault(key, []).append((str(row["code"]), float(row["cash_dividend"])))
    return out


def init_cutover_from_e22_v2(sdir: Path, e22_v2_dir: Path) -> dict:
    """Seed paper v3 challenger from live E22_v2; no rewrite of v2; no dividend backfill."""
    ...
    e21_state = json.loads((e22_v2_dir / "portfolio_state.json").read_text())
    ...
    state = {
        ...
        "cutover_from": "forward/e22_v2",
        "cutover_seed_date": seed_date,
        "cutover_note": "Inherited E22_v2 positions/cash; payment-date credits start after seed; no backfill",
    }
    ...
    pending_n = 0
    if (e22_v2_dir / "orders.csv").exists():
        orders = pd.read_csv(e22_v2_dir / "orders.csv", dtype={"code": str})
        filled = set()
        if (e22_v2_dir / "fills.csv").exists():
            filled = set(pd.read_csv(e22_v2_dir / "fills.csv").fill_id.astype(str))
    ...
    if (e22_v2_dir / "signals.csv").exists():
        sig = pd.read_csv(e22_v2_dir / "signals.csv")
    ...
    config = {
        "package": PACKAGE,
        "version_id": VERSION_ID,
        "status": "EXPERIMENTAL_PAPER",
        "governance_class": "EXPERIMENTAL_PAPER",
        "seeded_at": SEEDED_AT,
        "approval_phrase": APPROVAL_PHRASE,
        "modifies_e21": False,
        "preserves_e21_forever": True,
        "challenger_script": "scripts/e22_v3_challenger_forward_pipeline.py",
    ...
    (sdir / "README.md").write_text(
        f"""# E22_v3 Paper Challenger (EXPERIMENTAL)

**Version:** `{VERSION_ID}`  
**Status:** `EXPERIMENTAL_PAPER` (seeded {SEEDED_AT}) — **not** official
...
- Cutover seed date: `{seed_date}` (inherited from E22_v2; **no** dividend backfill)
...
def process_one_day(
    ...
    state = {
        ...
        "package": PACKAGE,
        "version_id": VERSION_ID,
        "cutover_from": state.get("cutover_from", "forward/e22_v2"),
...
def run_qc(sdir: Path) -> dict:
    ...
    if (sdir / "config.json").exists():
        cfg = json.loads((sdir / "config.json").read_text())
        cfg_ok = cfg.get("version_id") == VERSION_ID and cfg.get("status") == "EXPERIMENTAL_PAPER"
    checks["config_experimental_v3"] = cfg_ok
...
def main() -> None:
    ...
    ap.add_argument("--e22-v2-dir", default="forward/e22_v2")
    ...
    ap.add_argument(
        "--init-cutover-from-e22-v2",
        action="store_true",
        help="Seed forward/e22_v3_challenger from E22_v2 state (no dividend backfill)",
    )
    ap.add_argument(
        "--asof",
        default=None,
    ...
    if args.init_cutover_from_e22_v2:
        cfg = init_cutover_from_e22_v2(sdir, Path(args.e22_v2_dir))
        qc = run_qc(sdir)
    ...
    if not (sdir / "config.json").exists():
        raise RuntimeError("e22_v3_challenger not initialized; run with --init-cutover-from-e22-v2 first")
