#!/usr/bin/env python3
"""Stage A paper screen: dry-powder drawdown sleeve vs LIVE_STACK.

Charter: research/ops/DRY_POWDER_DRAWDOWN_CHARTER.md

Mechanism: hold 10% cash powder; deploy on observable PORT/TAIEX drawdown
gates; exit on fixed rules. Soft-Frozen + KD_OPT + TEL_EQUAL unchanged as
the 90% book. No wait-for-MDD. No live wire.
"""
from __future__ import annotations

import json
import sys
from datetime import datetime, timezone
from pathlib import Path

import numpy as np
import pandas as pd

sys.path.insert(0, str(Path(__file__).resolve().parent))
import e16_soft_frozen_base as soft
from e45_paper_harness import WINDOWS_STANDARD, load_dividends, load_market, window_stats
from e50_early_stack_combined_nav import FIN, e16_features, simulate_core
from portfolio_capital import DEFAULT_CAPITAL
from research_metric_helpers import cagr_delta_pp, mdd_delta_pp
from soft_assist_helpers import LIVE_KD
from tw_share_lots import BOARD_LOT
from within_sleeve_alloc import (
    FIN_PRE_EXDIV_KD,
    TEL_EQUAL,
    build_kd_season_tilt_scores,
    build_pre_exdiv_window_buy_ok,
)

ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "repro/dry-powder-drawdown"
OPS = ROOT / "research/ops"
CAPITAL = float(DEFAULT_CAPITAL)
LOT = BOARD_LOT
POWDER_W = 0.10
TRAIL_ALERT_PP = 3.0
TRAIL_PAUSE_PP = 5.0

# Stage A finite grid (≤40 books incl. live)
GATES = ("PORT_DD", "TAIEX_DD", "PORT_OR_TAIEX")
D_GRID = (0.10, 0.12, 0.15)
EXITS = (
    ("HOLD_40", "HOLD_N", 40),
    ("RECOVER_HALF", "RECOVER_HALF", None),
    ("RECOVER_FULL", "RECOVER_FULL", None),
    ("TIME_OR_HALF", "TIME_OR_HALF", 40),
)


def tip_gate(base_nav: pd.DataFrame, chal_nav: pd.DataFrame, asof: pd.Timestamp) -> dict:
    out = {}
    asof = pd.Timestamp(asof)
    b_dates = pd.to_datetime(base_nav["date"])
    c_dates = pd.to_datetime(chal_nav["date"])
    for wname, start in (
        ("ytd", pd.Timestamp(asof.year, 1, 1)),
        ("trailing_1y", asof - pd.Timedelta(days=365)),
    ):
        b = base_nav[(b_dates >= start) & (b_dates <= asof)].reset_index(drop=True)
        c = chal_nav[(c_dates >= start) & (c_dates <= asof)].reset_index(drop=True)
        if len(b) < 20 or len(c) < 20:
            out[wname] = {"giveback_pp": None, "gate": "INSUFFICIENT"}
            continue
        bn = b["nav"] / float(b["nav"].iloc[0])
        cn = c["nav"] / float(c["nav"].iloc[0])
        years = (len(b) - 1) / 252.0
        bc = float(bn.iloc[-1]) ** (1 / years) - 1 if years > 0 else None
        cc = float(cn.iloc[-1]) ** (1 / years) - 1 if years > 0 else None
        gb = None if bc is None or cc is None else (bc - cc) * 100
        gate = "PASS"
        if gb is not None and gb > TRAIL_PAUSE_PP:
            gate = "PAUSE_REVIEW"
        elif gb is not None and gb > TRAIL_ALERT_PP:
            gate = "ALERT"
        out[wname] = {
            "giveback_pp": None if gb is None else float(gb),
            "gate": gate,
            "rel_nav": float(cn.iloc[-1] / bn.iloc[-1]),
        }
    return out


def score_vs_base(base_stats: dict, chal_stats: dict) -> dict:
    mdd_pp = mdd_delta_pp(base_stats.get("max_drawdown"), chal_stats.get("max_drawdown"))
    cagr_pp = cagr_delta_pp(
        base_stats.get("cagr"), chal_stats.get("cagr"), missing_as_zero=True
    )
    giveback = abs(float(cagr_pp)) if cagr_pp is not None else 9.0
    return {
        "mdd_improve_pp": float(mdd_pp),
        "cagr_giveback_pp": float(cagr_pp) if cagr_pp is not None else None,
        "score": float(mdd_pp) - 0.5 * giveback,
    }


def running_drawdown(nav: pd.Series) -> pd.Series:
    peak = nav.cummax()
    return nav / peak.replace(0.0, np.nan) - 1.0


def sim_live_stack(market, target, regime, dividends, scores, buy_ok, schedule=None):
    return simulate_core(
        market,
        target,
        regime,
        dividends,
        apply_e22=True,
        apply_stock_div=True,
        capital=CAPITAL,
        lot_size=LOT,
        financial_alloc=FIN_PRE_EXDIV_KD,
        telecom_alloc=TEL_EQUAL,
        fin_name_scores=scores,
        fin_buy_ok=buy_ok,
        sleeve_weight_schedule=schedule,
    )


def build_powder_schedule(
    target: pd.DataFrame,
    *,
    gate_id: str,
    d_thresh: float,
    exit_kind: str,
    hold_n: int | None,
    port_dd: pd.Series,
    taiex_dd: pd.Series,
) -> tuple[pd.DataFrame, dict]:
    """Build Soft-Frozen sleeve schedule with 10% cash powder idle / deploy-to-FIN."""
    dates = list(target.index)
    port_dd = port_dd.reindex(dates)
    taiex_dd = taiex_dd.reindex(dates)

    rows = []
    deployed = False
    trigger_peak_nav_dd = None  # dd depth at trigger (negative)
    enter_i = None
    n_entries = 0
    n_days_deployed = 0

    for i, dt in enumerate(dates):
        # Causal: use prior-day DD (i-1); day 0 never enters
        if i == 0:
            fire = False
        else:
            pdd = float(port_dd.iloc[i - 1]) if pd.notna(port_dd.iloc[i - 1]) else 0.0
            tdd = float(taiex_dd.iloc[i - 1]) if pd.notna(taiex_dd.iloc[i - 1]) else 0.0
            port_hit = pdd <= -float(d_thresh)
            taiex_hit = tdd <= -float(d_thresh)
            if gate_id == "PORT_DD":
                fire = port_hit
            elif gate_id == "TAIEX_DD":
                fire = taiex_hit
            elif gate_id == "PORT_OR_TAIEX":
                fire = port_hit or taiex_hit
            else:
                raise ValueError(gate_id)

        base = target.loc[dt, ["Financial", "Telecom", "0050"]].astype(float).to_numpy()
        idle = base * (1.0 - POWDER_W)

        if not deployed:
            if fire:
                deployed = True
                enter_i = i
                n_entries += 1
                # depth from the signal day
                if gate_id == "TAIEX_DD":
                    trigger_peak_nav_dd = float(taiex_dd.iloc[i - 1])
                else:
                    # PORT or OR: prefer port depth when available
                    trigger_peak_nav_dd = float(port_dd.iloc[i - 1]) if pd.notna(port_dd.iloc[i - 1]) else float(
                        taiex_dd.iloc[i - 1]
                    )
                # deploy: idle + 0.10 to Financial, then Soft-Frozen clip
                dep = idle.copy()
                dep[0] = float(dep[0] + POWDER_W)
                dep = soft.apply_soft_frozen_clips(dep)
                rows.append(dep)
                n_days_deployed += 1
            else:
                rows.append(idle)
        else:
            # check exit using prior-day info / holding days
            held = i - int(enter_i)
            pdd = float(port_dd.iloc[i - 1]) if i > 0 and pd.notna(port_dd.iloc[i - 1]) else 0.0
            exit_now = False
            if exit_kind == "HOLD_N":
                exit_now = held >= int(hold_n)
            elif exit_kind == "RECOVER_HALF":
                # recovered half of the (negative) depth → dd > trigger/2
                half = float(trigger_peak_nav_dd) * 0.5
                exit_now = pdd >= half
            elif exit_kind == "RECOVER_FULL":
                exit_now = pdd >= 0.0  # back to peak on live-nav path
            elif exit_kind == "TIME_OR_HALF":
                half = float(trigger_peak_nav_dd) * 0.5
                exit_now = held >= int(hold_n) or pdd >= half
            else:
                raise ValueError(exit_kind)

            if exit_now:
                deployed = False
                enter_i = None
                trigger_peak_nav_dd = None
                rows.append(idle)
            else:
                dep = idle.copy()
                dep[0] = float(dep[0] + POWDER_W)
                dep = soft.apply_soft_frozen_clips(dep)
                rows.append(dep)
                n_days_deployed += 1

    sched = pd.DataFrame(rows, index=dates, columns=["Financial", "Telecom", "0050"])
    meta = {
        "n_entries": int(n_entries),
        "n_days_deployed": int(n_days_deployed),
        "deploy_frac": float(n_days_deployed / max(len(dates), 1)),
    }
    return sched, meta


def main() -> int:
    OUT.mkdir(parents=True, exist_ok=True)
    (OUT / "reports").mkdir(exist_ok=True)
    OPS.mkdir(parents=True, exist_ok=True)
    assert soft.SOFT_FROZEN_FIN_CLIP == [0.6, 0.9]

    import e21_forward_pipeline as e21

    for k in ("season_start", "season_end", "k_thresh", "pre_days", "active_score"):
        if LIVE_KD[k] != e21.KD_OPT[k]:
            raise SystemExit(f"LIVE_KD[{k}] drift vs e21.KD_OPT")
    if e21.LIVE_E45_STITCH:
        raise SystemExit("Refuse dry-powder screen while LIVE_E45_STITCH is True")

    print("loading ...", flush=True)
    market = load_market()
    dividends = load_dividends()
    _p, _s, target, regime = e16_features(market)
    cal = pd.DatetimeIndex(pd.to_datetime(market["date"]).drop_duplicates().sort_values())

    kd_scores = build_kd_season_tilt_scores(
        market,
        dividends,
        FIN,
        k_thresh=float(LIVE_KD["k_thresh"]),
        season_start=LIVE_KD["season_start"],
        season_end=LIVE_KD["season_end"],
        pre_days=int(LIVE_KD["pre_days"]),
        active_score=float(LIVE_KD["active_score"]),
    )
    kd_ok = build_pre_exdiv_window_buy_ok(
        cal, dividends, FIN, pre_days=int(LIVE_KD["pre_days"]), also_stock_ex=True
    )

    print("LIVE_STACK ...", flush=True)
    nav_live, fills_live, meta_live = sim_live_stack(
        market, target, regime, dividends, kd_scores, kd_ok
    )
    assert meta_live.get("exact_t1_ok")
    win_live = {w: window_stats(nav_live, a, b) for w, (a, b) in WINDOWS_STANDARD.items()}
    asof = pd.to_datetime(nav_live["date"]).max()

    live_nav_s = (
        nav_live.assign(date=pd.to_datetime(nav_live["date"]))
        .drop_duplicates("date")
        .set_index("date")["nav"]
        .astype(float)
        .reindex(target.index)
        .ffill()
    )
    port_dd = running_drawdown(live_nav_s)

    prices = (
        market.pivot(index="date", columns="code", values="adj_close")
        .sort_index()
        .ffill()
    )
    taiex = prices["TAIEX"].reindex(target.index).ffill()
    taiex_dd = running_drawdown(taiex.astype(float))

    jobs = []
    for gate in GATES:
        for d in D_GRID:
            for exit_id, exit_kind, hold_n in EXITS:
                d_tag = str(int(d * 100))
                book_id = f"DP10_{gate}_D{d_tag}_{exit_id}"
                jobs.append((book_id, gate, float(d), exit_id, exit_kind, hold_n))

    print(f"Stage A challengers: {len(jobs)} (+ LIVE_STACK)", flush=True)
    rows = []

    # live row: tip vs self trivial; score vs self = 0
    live_tip = tip_gate(nav_live, nav_live, asof)
    rows.append(
        {
            "id": "LIVE_STACK",
            "gate": "none",
            "d_thresh": None,
            "exit": "none",
            "heldout_score": 0.0,
            "mdd_improve_pp": 0.0,
            "cagr_giveback_pp": 0.0,
            "tip_ytd": live_tip["ytd"]["gate"],
            "tip_1y": live_tip["trailing_1y"]["gate"],
            "tip_clean": True,
            "coexist": False,
            "vs_live_heldout_delta": 0.0,
            "full_cagr": win_live["full"].get("cagr"),
            "full_mdd": win_live["full"].get("max_drawdown"),
            "heldout_cagr": win_live["heldout_2019_plus"].get("cagr"),
            "heldout_mdd": win_live["heldout_2019_plus"].get("max_drawdown"),
            "sealed_cagr": win_live["sealed_2023_plus"].get("cagr"),
            "sealed_mdd": win_live["sealed_2023_plus"].get("max_drawdown"),
            "n_fills": int(len(fills_live)),
            "n_entries": 0,
            "deploy_frac": 0.0,
        }
    )

    for i, (book_id, gate, d_thresh, exit_id, exit_kind, hold_n) in enumerate(jobs, 1):
        if i == 1 or i % 6 == 0 or i == len(jobs):
            print(f"  [{i}/{len(jobs)}] {book_id}", flush=True)
        sched, smeta = build_powder_schedule(
            target,
            gate_id=gate,
            d_thresh=d_thresh,
            exit_kind=exit_kind,
            hold_n=hold_n,
            port_dd=port_dd,
            taiex_dd=taiex_dd,
        )
        nav, fills, meta = sim_live_stack(
            market, target, regime, dividends, kd_scores, kd_ok, schedule=sched
        )
        assert meta.get("exact_t1_ok"), book_id
        win = {w: window_stats(nav, a, b) for w, (a, b) in WINDOWS_STANDARD.items()}
        tip = tip_gate(nav_live, nav, asof)
        held = score_vs_base(win_live["heldout_2019_plus"], win["heldout_2019_plus"])
        tip_clean = tip["ytd"]["gate"] == "PASS" and tip["trailing_1y"]["gate"] == "PASS"
        coexist = bool(tip_clean and held["score"] > 0)
        rows.append(
            {
                "id": book_id,
                "gate": gate,
                "d_thresh": d_thresh,
                "exit": exit_id,
                "heldout_score": float(held["score"]),
                "mdd_improve_pp": float(held["mdd_improve_pp"]),
                "cagr_giveback_pp": held["cagr_giveback_pp"],
                "tip_ytd": tip["ytd"]["gate"],
                "tip_1y": tip["trailing_1y"]["gate"],
                "tip_clean": tip_clean,
                "coexist": coexist,
                "vs_live_heldout_delta": float(held["score"]),
                "full_cagr": win["full"].get("cagr"),
                "full_mdd": win["full"].get("max_drawdown"),
                "heldout_cagr": win["heldout_2019_plus"].get("cagr"),
                "heldout_mdd": win["heldout_2019_plus"].get("max_drawdown"),
                "sealed_cagr": win["sealed_2023_plus"].get("cagr"),
                "sealed_mdd": win["sealed_2023_plus"].get("max_drawdown"),
                "n_fills": int(len(fills)),
                "n_entries": smeta["n_entries"],
                "deploy_frac": smeta["deploy_frac"],
            }
        )

    def rank_key(r):
        beat = bool(r.get("coexist") and r["id"] != "LIVE_STACK" and r.get("vs_live_heldout_delta", 0) > 0)
        no_pause = r["tip_ytd"] != "PAUSE_REVIEW" and r["tip_1y"] != "PAUSE_REVIEW"
        return (1 if r.get("coexist") else 0, 1 if beat else 0, 1 if no_pause else 0, r["heldout_score"])

    ranked = sorted(rows, key=rank_key, reverse=True)
    beat = [r for r in ranked if r["id"] != "LIVE_STACK" and r.get("coexist") and r["vs_live_heldout_delta"] > 0]
    near = [
        r
        for r in ranked
        if r["id"] != "LIVE_STACK" and r.get("tip_clean") and r.get("vs_live_heldout_delta", -9) > -0.05
    ]
    coexist_other = [r for r in ranked if r["id"] != "LIVE_STACK" and r.get("coexist")]
    if beat:
        verdict = "BEATS_LIVE"
    elif near:
        verdict = "NEAR_NO_BEAT"
    elif coexist_other:
        verdict = "COEXIST_NO_LIFT"
    else:
        verdict = "NO_LIFT"

    payload = {
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "label": "DRY_POWDER_DRAWDOWN_SCREEN_STAGE_A",
        "charter": "research/ops/DRY_POWDER_DRAWDOWN_CHARTER.md",
        "status": "PAPER_SCREEN_DONE",
        "live_wire": False,
        "asof": str(pd.Timestamp(asof).date()),
        "powder_w": POWDER_W,
        "n_books": len(rows),
        "n_challengers": len(jobs),
        "n_beat_live": len(beat),
        "beat_live_ids": [r["id"] for r in beat[:40]],
        "near_ids": [r["id"] for r in near[:40]],
        "top20": [r["id"] for r in ranked[:20]],
        "verdict": verdict,
        "stage_a_grid": {
            "gates": list(GATES),
            "d_thresh": list(D_GRID),
            "exits": [e[0] for e in EXITS],
        },
        "books": ranked,
        "non_actions": [
            "No Soft-Frozen flip",
            "No live KD/TEL change",
            "No E45 stitch",
            "No Soft-assist live wire",
            "No wait-for-MDD",
        ],
    }

    pd.DataFrame(ranked).to_csv(OUT / "reports" / "scoreboard.csv", index=False)
    (OUT / "reports" / "dry_powder_drawdown_screen.json").write_text(
        json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
    )
    (OPS / "DRY_POWDER_DRAWDOWN_SCREEN.json").write_text(
        json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
    )

    def pct(x):
        return "—" if x is None else f"{100*float(x):.2f}%"

    def line(r):
        return (
            f"| `{r['id']}` | {r['gate']} | {r['exit']} | {r['tip_ytd']} | {r['tip_1y']} | "
            f"{r['heldout_score']:.3f} | {r['mdd_improve_pp']:.3f} | "
            f"{pct(r['heldout_cagr'])} | {pct(r['heldout_mdd'])} | "
            f"{pct(r['sealed_cagr'])} | {pct(r['sealed_mdd'])} | "
            f"{r['n_entries']} | {r['deploy_frac']:.2%} |"
        )

    lines = [
        "# Dry-Powder Drawdown Screen — Stage A",
        "",
        f"Generated: `{payload['generated_at_utc']}` · asof **{payload['asof']}**",
        f"Charter: `DRY_POWDER_DRAWDOWN_CHARTER.md`",
        f"Verdict: **`{verdict}`** · powder **{POWDER_W:.0%}** · books **{payload['n_books']}**",
        "",
        "## Question",
        "",
        "Does 10% dry powder + observable PORT/TAIEX drawdown entry + fixed exit tip-clean beat **LIVE_STACK**?",
        "",
        "## Summary",
        "",
        f"- Beat live: **{payload['n_beat_live']}** → `{payload['beat_live_ids'][:12]}`",
        f"- Near (tip-clean, score &gt; −0.05): `{payload['near_ids'][:12]}`",
        f"- Grid: gates `{list(GATES)}` · D `{list(D_GRID)}` · exits `{[e[0] for e in EXITS]}`",
        "",
        "## Top 20",
        "",
        "| ID | Gate | Exit | Tip YTD | Tip 1y | Held score | MDDΔpp | Held CAGR | Held MDD | Sealed CAGR | Sealed MDD | Entries | Deploy% |",
        "|---|---|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    lines.extend(line(r) for r in ranked[:20])
    lines += [
        "",
        "## LIVE_STACK reference",
        "",
        f"- full CAGR/MDD: {pct(rows[0]['full_cagr'])} / {pct(rows[0]['full_mdd'])}",
        f"- heldout CAGR/MDD: {pct(rows[0]['heldout_cagr'])} / {pct(rows[0]['heldout_mdd'])}",
        f"- sealed CAGR/MDD: {pct(rows[0]['sealed_cagr'])} / {pct(rows[0]['sealed_mdd'])}",
        "",
        "## Reading",
        "",
        "- Held score = MDD↑pp − 0.5·|CAGRΔpp| vs **LIVE_STACK** (not FIN_EQUAL).",
        "- Sealed is report-only in Stage A.",
        "- If verdict is NO_LIFT / NEAR_NO_BEAT → keep live; STOP or archive unless human expands grid.",
        "",
        "## Non-actions",
        "",
        "- No Soft-Frozen / live KD / TEL / E45 / Soft-assist wire from this screen",
        "",
        "## Label",
        "",
        "`DRY_POWDER_DRAWDOWN_SCREEN_STAGE_A_2026-09-10`",
        "",
    ]
    md = "\n".join(lines)
    (OPS / "DRY_POWDER_DRAWDOWN_SCREEN.md").write_text(md, encoding="utf-8")
    (OUT / "reports" / "DRY_POWDER_DRAWDOWN_SCREEN.md").write_text(md, encoding="utf-8")

    zh = "\n".join(
        [
            "# 乾粉回撤 Screen — Stage A（paper）",
            "",
            f"產生：`{payload['generated_at_utc']}` · asof **{payload['asof']}**",
            f"總評：**`{verdict}`** · 乾粉 **10%** · books **{payload['n_books']}**",
            f"- 勝過 live：**{payload['n_beat_live']}** → `{payload['beat_live_ids'][:10]}`",
            f"- Near：`{payload['near_ids'][:10]}`",
            "",
            "Live Soft-Frozen / KD_OPT / TEL **KEEP**。不是等 MDD。",
            "",
            "英文：`DRY_POWDER_DRAWDOWN_SCREEN.md`",
            "",
        ]
    )
    (OPS / "DRY_POWDER_DRAWDOWN_SCREEN.zh-TW.md").write_text(zh, encoding="utf-8")

    # mark charter paper done
    print(
        json.dumps(
            {
                "verdict": verdict,
                "n_books": payload["n_books"],
                "n_beat_live": payload["n_beat_live"],
                "beat_live_ids": payload["beat_live_ids"][:15],
                "top10": payload["top20"][:10],
            },
            ensure_ascii=False,
            indent=2,
        )
    )
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
