#!/usr/bin/env python3
"""E45 M2 PAPER — true DEF sleeve relocate (00719B / BIL×FX) on Exact T+1 early-stack.

Frozen v1: research/e45/E45_M2_DEF_SLEEVE_V1_FROZEN.md
Sensor: M1 frozen intensity s_{t-1} (M1 Section-2 FAIL does not block M2).
Refs: SHRINK + RELOC_TEL; challengers: RELOC_719B / RELOC_BIL_FX / HYBRID_719B.
...
Does NOT merge DEF into forward/e21/live_market.csv.
"""
...
OUT = ROOT / "repro/e45-m2-true-def-relocate"
DEF_DIR = ROOT / "data/def_proxies"
RESEARCH = ROOT / "research/e45"
OPS = ROOT / "research/ops"

DEF_SLEEVE_LABEL = "E45_M2_DEF_SLEEVE_V1_FROZEN_2026-09-06"
LISTING_719B = __import__("pandas").Timestamp("2018-02-01")
CODE_BIL_FX = "BIL_FX"
CODE_719B_SW = "DEF_719B_SW"
COST_MULTS = (0, 1, 2, 3)
...
def _tr_level(adj: pd.Series, start: float = 100.0) -> pd.Series:
    ...
    return (1.0 + rets).cumprod() * float(start)


def build_def_bars(equity_cal: pd.DatetimeIndex) -> pd.DataFrame:
    """BIL_FX and 719B-switch total-return bars on the equity calendar."""
    import numpy as np
    ...
    b719 = _load_ohlcv(DEF_DIR / "00719B_ohlcv.csv").set_index("date")
    cal = pd.DatetimeIndex(sorted(equity_cal))
    bil_twd = bil["adj_close"].reindex(cal).ffill() * fx["usdtwd_mid"].reindex(cal).ffill()
    bil_lvl = _tr_level(bil_twd)
    r_bil = bil_twd.pct_change().fillna(0.0)
    r_719 = b719["adj_close"].reindex(cal).ffill().pct_change().fillna(0.0)
    active = pd.Series(np.where(cal >= LISTING_719B, r_719.to_numpy(), r_bil.to_numpy()), index=cal)
    sw_lvl = (1.0 + active).cumprod() * 100.0
    rows = []
    for code, lvl in ((CODE_BIL_FX, bil_lvl), (CODE_719B_SW, sw_lvl)):
        prev = lvl.shift(1)
        for dt, close in lvl.items():
            ...
            rows.append({
                "date": dt, "code": code, "open": o,
                "high": max(o, float(close)), "low": min(o, float(close)),
                "close": float(close), "adj_close": float(close), "volume": 0.0,
            })
...
def main() -> None:
    ...
    print("building true-DEF bars ...", flush=True)
    def_bars = build_def_bars(pd.DatetimeIndex(sorted(market["date"].unique())))
    def_bars.to_csv(OUT / "outputs" / "m2v1_def_proxy_bars.csv", index=False)
    ...
    m2_specs = [
        ...
        ("M2_RELOC_719B_C50", "RELOC_719B", 0.50, "m2_reloc_719b", CODE_719B_SW),
        ("M2_RELOC_719B_C75", "RELOC_719B", 0.75, "m2_reloc_719b", CODE_719B_SW),
        ("M2_RELOC_BIL_FX_C50", "RELOC_BIL_FX", 0.50, "m2_reloc_bil_fx", CODE_BIL_FX),
        ("M2_RELOC_BIL_FX_C75", "RELOC_BIL_FX", 0.75, "m2_reloc_bil_fx", CODE_BIL_FX),
        ("M2_HYBRID_719B_C50", "HYBRID_719B", 0.50, "m2_hybrid_719b", CODE_719B_SW),
        ("M2_HYBRID_719B_C75", "HYBRID_719B", 0.75, "m2_hybrid_719b", CODE_719B_SW),
    ]
    books: list[dict] = [
        {
            "book": BOOK_BASE,
    ...
    true_kinds = {"m2_reloc_719b", "m2_reloc_bil_fx", "m2_hybrid_719b"}
    true_mask = qual_df.kind.astype(str).isin(true_kinds)
    true_qualifiers = qual_df[true_mask & qual_df.qualifies_section2]["book"].tolist()
    any_true_pass = len(true_qualifiers) > 0

    shrink = delta_df[
        (delta_df.book == "M2_SHRINK_C50")
        & (delta_df.cost_multiple == 1)
        & (delta_df.window == "heldout_2019_plus")
    ].iloc[0]
    tel = delta_df[
        (delta_df.book == "M2_RELOC_TEL_C50")
        & (delta_df.cost_multiple == 1)
        & (delta_df.window == "heldout_2019_plus")
    ].iloc[0]
    pass_vs_refs: list[dict] = []
    for book in qual_df[true_mask]["book"].tolist():
        chal = delta_df[
            (delta_df.book == book)
            & (delta_df.cost_multiple == 1)
            & (delta_df.window == "heldout_2019_plus")
        ...
        beats_tel = (
            chal["covid_ex_score"] is not None
            and tel["covid_ex_score"] is not None
            and float(chal["covid_ex_score"]) > float(tel["covid_ex_score"])
        )
        ...
        pass_vs_refs.append(
            {
                ...
                "tel_covid_ex_score": tel["covid_ex_score"],
                "beats_shrink_covid_ex": beats_shrink,
                "beats_tel_covid_ex": beats_tel,
                "rule4_or_5": rule4_or_5,
                "m2v1_stage_pass": bool(beats_shrink and rule4_or_5),
                "qualifies_section2": bool(qrow["qualifies_section2"]),
            }
        )
    stage_pass_df = pd.DataFrame(pass_vs_refs)
    stage_pass_df.to_csv(OUT / "outputs" / "m2v1_stage_pass_vs_refs.csv", index=False)
    any_stage_pass = bool(stage_pass_df["m2v1_stage_pass"].any()) if len(stage_pass_df) else False

    preferred = None
    true_held = held1[held1.kind.astype(str).isin(true_kinds)]
    if not true_held.empty:
        preferred = true_held.iloc[0].to_dict()

    # Freeze: best true-DEF clears §2, else autopsy. Stage gate is informational.
    verdict = "PASS" if any_true_pass else "FAIL_AUTOPSY"
    payload = {
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "status": "PAPER_ONLY",
        "ballot": "E45 M2 v1 true-DEF sleeve relocate paper screen",
        "def_sleeve": DEF_SLEEVE_LABEL,
        "honesty": (
            "DEF_719B_SW=BIL_FX pre-2018-02-01 then 00719B TR; "
            "BIL_FX=BIL adj*USDTWD mid (FX risk; mid optimistic); "
            "not merged into live_market; DEF_TEL rebuild is equity proxy ref only"
        ),
        ...
        "m2v1_stage_pass_vs_refs": pass_vs_refs,
        "true_def_section2_qualifiers": true_qualifiers,
        "true_def_section2_any_pass": any_true_pass,
    ...
    (OUT / "reports" / "e45_m2_true_def_relocate.json").write_text(
        json.dumps(payload, indent=2, default=str) + "\n"
    )
    (RESEARCH / "E45_M2_TRUE_DEF_RELOCATE.json").write_text(
        json.dumps(payload, indent=2, default=str) + "\n"
    )
    ...
    for r in pass_vs_refs:
        lines.append(
            f"| {r['book']} | {yn(r['beats_shrink_covid_ex'])} | {yn(r['beats_tel_covid_ex'])} | "
            f"{yn(r['rule4_or_5'])} | {yn(r['m2v1_stage_pass'])} | {yn(r['qualifies_section2'])} |"
        )
    lines += [
        "",
    ...
    md = "\n".join(lines) + "\n"
    (OUT / "reports" / "e45_m2_true_def_relocate.md").write_text(md)
    (RESEARCH / "E45_M2_TRUE_DEF_RELOCATE.md").write_text(md)
    (OPS / "E45_M2_TRUE_DEF_RELOCATE.md").write_text(
        "\n".join(
            [
                "# Ops pointer — E45 M2 v1 true DEF relocate (paper)",
                "",
                f"Verdict: **{verdict}**",
                "",
                "- Research: `research/e45/E45_M2_TRUE_DEF_RELOCATE.md`",
                "- Freeze: `research/e45/E45_M2_DEF_SLEEVE_V1_FROZEN.md`",
    ...
    print(f"DONE verdict={verdict} true_def_section2={any_true_pass} stage_pass={any_stage_pass} qualifiers={true_qualifiers}", flush=True)
