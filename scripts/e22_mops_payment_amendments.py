#!/usr/bin/env python3
"""MOPS payment-date amendment overlay (D4 / Gap 6.9b).
...
Schema ``mops_payment_amendments.csv``:
  code,original_payment_date,amended_payment_date,source,note
"""
...
# code + original ISO pay → amended ISO pay
AmendmentMap = dict[tuple[str, str], str]

_AMEND_PATTERNS = (
    # 原訂…發放日…2026年7月10日…顺延…7月13日
    re.compile(
        r"原[訂定].{0,24}?(?:發放|給付|配發).{0,40}?"
    ...
    re.compile(
        r"原(?:訂)?(?:發放|給付)?日?\s*(?P<o>\d{4}-\d{2}-\d{2})"
...
def lookup_amendment(
    ...
    key = (str(code).strip(), str(original_payment_date).strip()[:10])
    raw = amendments.get(key)
    if not raw:
        return None
    try:
...
def merge_amendment_rows(*groups: Iterable[Mapping[str, str]]) -> list[dict[str, str]]:
    ...
    return sorted(by_key.values(), key=lambda x: (x["code"], x["original_payment_date"]))
...
def fetch_mops_t108_html(roc_year: int) -> str:
