...
OUT = ROOT / "repro/mdd-loss-engine/l3_oof"
...
CANDIDATES: list[dict] = [
    ...
    {"id": "L3_MILD_35_60", "family": "L3-FINCAP-MILD", "kind": "fin_cap",
     "params": {"fin_lo": 0.35, "fin_hi": 0.60}, "lockable": True},
    {"id": "L3_MILD_35_70", "family": "L3-FINCAP-MILD", "kind": "fin_cap",
     "params": {"fin_lo": 0.35, "fin_hi": 0.70}, "lockable": True},
    {"id": "L3_MILD_35_80", "family": "L3-FINCAP-MILD", "kind": "fin_cap",
     "params": {"fin_lo": 0.35, "fin_hi": 0.80}, "lockable": True},
    {"id": "L3_MILD_50_60", "family": "L3-FINCAP-MILD", "kind": "fin_cap",
     "params": {"fin_lo": 0.50, "fin_hi": 0.60}, "lockable": True},
    {"id": "L3_MILD_50_70", "family": "L3-FINCAP-MILD", "kind": "fin_cap",
     "params": {"fin_lo": 0.50, "fin_hi": 0.70}, "lockable": True},
    {"id": "L3_MILD_50_80", "family": "L3-FINCAP-MILD", "kind": "fin_cap",
     "params": {"fin_lo": 0.50, "fin_hi": 0.80}, "lockable": True},
    {"id": "L3_BLEND_025", "family": "L3-FINCAP-BLEND", "kind": "blend",
     "params": {"alpha": 0.25}, "lockable": True},
    {"id": "L3_BLEND_050", "family": "L3-FINCAP-BLEND", "kind": "blend",
     "params": {"alpha": 0.50}, "lockable": True},
    {"id": "L3_BLEND_075", "family": "L3-FINCAP-BLEND", "kind": "blend",
     "params": {"alpha": 0.75}, "lockable": True},
]
...
def main() -> None:
    ...
    mild_cache: dict[tuple[float, float], tuple[pd.DataFrame, pd.Series]] = {
    ...
    for cand in CANDIDATES:
        ...
        if kind == "base":
            ...
            if key not in mild_cache:
                _px, _sl, tgt, reg = oof.e16_features_fin_cap(market, key[0], key[1])
                mild_cache[key] = (tgt, reg)
            target, regime = mild_cache[key]
        elif kind == "blend":
        ...
        if cid in {"BASE", "L2_FINCAP_ONLY", "L3_MILD_50_70", "L3_BLEND_050"}:
        ...
        scored.append(
            {
                **r,
                ...
                "late_bull_cagr_giveback_pp": late_gb * 100.0,
    ...
    decision = "OOF_L3_READY_FOR_ADV_LITE" if winner else "STOP_L3_OOF_NO_PASSER"
    ...
    summary = {
        ...
        "screen_families": ["L3-FINCAP-MILD", "L3-FINCAP-BLEND"],
        "deferred_families": ["L3-FINCAP-BULL-RESTORE", "L3-FINCAP-DD-ONLY"],
        "base": {
        ...
        "winner_row": winner,
        "next_if_pass": "adversarial-lite (placebo FIN-cap intensity + year-split + late-bull); then one held-out",
        "next_if_stop": "keep BASE; do not retune L1/L2; optionally screen BULL-RESTORE / DD-ONLY under same charter",
    }

    (OUT / "reports" / "l3_oof_summary.json").write_text(json.dumps(summary, indent=2) + "\n")
    (RESEARCH / "MDD_L3_OOF_SUMMARY.json").write_text(json.dumps(summary, indent=2) + "\n")
    ...
    for r in scored:
        ...
        flat.append(item)
    pd.DataFrame(flat).to_csv(OUT / "outputs" / "l3_oof_candidates.csv", index=False)
    ...
    for r in scored:
        ...
        lines.append(
            ...
            f"{r['mean_fin_w_oof']:.1%} | {r['exact_t1_ok']} | {pass_cell} | {fail} |"
        )
    ...
    if winner:
        ...
        lines += [
            "- **STOP OOF** — no lockable MILD/BLEND passer under tightened sealed-aware gates.",
            "- Keep BASE. Do not retune L1/L2 locks.",
            "- Optional under same charter: screen BULL-RESTORE / DD-ONLY (deferred families).",
        ]
    lines += [
        "",
        "Artifacts:",
        f"- `{OUT / 'reports/l3_oof_summary.json'}`",
        f"- `{OUT / 'outputs/l3_oof_candidates.csv'}`",
        "",
    ]
    md = "\n".join(lines)
    (OUT / "MDD_L3_OOF.md").write_text(md)
    (RESEARCH / "MDD_L3_OOF.md").write_text(md)
