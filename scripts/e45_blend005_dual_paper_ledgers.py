#!/usr/bin/env python3
"""E45 blend-α=0.05 dual-paper ledgers (OPERATING OBSERVE — not live).

Side-by-side Exact T+1 paper books:
  BASE_E16_E18_E22_v2s — Soft-Frozen early-stack + formal E22_v2s
  BLEND_E45_A05       — same stack + 5% intensity E45 E3_VOLTARGET_WINNER overlay

Opened by human ballot: ``E45 OPEN blend-α=0.05 observe``.
...
def main() -> None:
    ...
    e45_blend = e45_blend.rename("e45_blend_a05_exposure")
    ...
    nav_c.to_csv(OUT / "outputs" / "blend_e45_a05_daily_nav.csv", index=False)
    fills_b.to_csv(OUT / "outputs" / "base_e16_e18_e22_v2s_fills.csv", index=False)
    fills_c.to_csv(OUT / "outputs" / "blend_e45_a05_fills.csv", index=False)
    base_target.to_csv(OUT / "outputs" / "base_targets.csv")
    e45_blend.to_csv(OUT / "outputs" / "blend_e45_a05_exposure.csv")

    jb = nav_b[["date", "nav"]].rename(columns={"nav": "nav_base"})
    jc = nav_c[["date", "nav"]].rename(columns={"nav": "nav_blend_e45_a05"})
    joined = jb.merge(jc, on="date", how="inner")
    joined["rel_blend_vs_base"] = joined["nav_blend_e45_a05"] / joined["nav_base"]
    ...
    proposal = {
        ...
        "exact_t1": {
            ...
            "blend_e45_a05": books[CHAL_ID]["exact_t1_ok"],
        },
        "books": books,
    ...
    (OUT / "reports" / "e45_blend005_dual_paper_observe.json").write_text(
        json.dumps(proposal, indent=2) + "\n"
    )
    (RESEARCH / "E45_BLEND005_DUAL_PAPER_OBSERVE.json").write_text(json.dumps(proposal, indent=2) + "\n")
    ...
    lines = [
        ...
        f"- **{CHAL_ID}**: same stack + α=0.05 × E45 `{E45_PROFILE_DEFAULT}` exposure overlay",
    ...
    lines += [
        ...
        "",
        "Artifacts:",
        f"- `{OUT / 'reports' / 'e45_blend005_dual_paper_observe.json'}`",
        f"- `{OUT / 'outputs' / 'dual_paper_nav_compare.csv'}`",
        "",
    ]
    md = "\n".join(lines)
    (OUT / "E45_BLEND005_DUAL_PAPER_OBSERVE.md").write_text(md)
    (RESEARCH / "E45_BLEND005_DUAL_PAPER_OBSERVE.md").write_text(md)
    print(
        json.dumps(
            {
