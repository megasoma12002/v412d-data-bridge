#!/usr/bin/env python3
"""V4.12-E10S2: official TAIEX 1000-point shocks and 0050 deployment tests."""
...
def main():
    ...
    ret=pd.concat([fc.pct_change(fill_method=None),tc.pct_change(fill_method=None),adj0050.pct_change(fill_method=None).rename('0050')],axis=1).fillna(0);fr=ret[FIN].mean(axis=1);tr=ret[TEL].mean(axis=1);er=ret['0050'];point_drop=ta.close.diff();pct=ta.close.pct_change(fill_method=None);shock=(point_drop<=-1000);shock_guarded=shock&(pct<=-.02)
    ta.assign(point_change=point_drop,pct_change=pct,shock_1000=shock,shock_guarded=shock_guarded).to_csv(out/'e10s2_taiex.csv')
    selected=[];last=-99
    for i,d in enumerate(dates):
        if shock_guarded.loc[d] and i-last>=10:selected.append((i,d,float(point_drop.loc[d]),float(pct.loc[d])));last=i
    study=[]
    for i,d,p,pr in selected:
        for h in [1,5,10,20]:
            if i+h>=len(dates):continue
            vals={k:(1+s.iloc[i+1:i+h+1]).prod()-1 for k,s in [('financial',fr),('telecom',tr),('0050',er)]};study.append({'date':d,'point_drop':p,'taiex_return':pr,'horizon':h,**vals,'financial_minus_telecom':vals['financial']-vals['telecom'],'0050_minus_telecom':vals['0050']-vals['telecom']})
    ...
    status={'version':'V4.12-E10S2','shock_rule':'TAIEX close point change <= -1000 and return <= -2%; 10-session cooldown','raw_shock_days':int(shock.sum()),'guarded_shock_days':int(shock_guarded.sum()),'independent_events':len(selected),'0050_qc':q0050,'integration':'forward shadow monitor only; small and level-dependent sample'};(out/'e10s2_status.json').write_text(json.dumps(status,indent=2)+'\n');print(summary.round(4).to_string(index=False));print(res[['model','split','cagr_delta','sharpe_delta','max_drawdown']].round(4).to_string(index=False));print(json.dumps(status,indent=2))
