#!/usr/bin/env python3
"""Financial within-sleeve allocation — RESEARCH ONLY (Stage B).

Charter: research/ops/FIN_WITHIN_SLEEVE_ALLOC_CHARTER.md
Ballot: 金融也研究分開 (ACCEPT charter)

- Soft-Frozen sleeve clips untouched.
- Live e21 NOT wired.
- Telecom held at TEL_EQUAL (isolate FIN).
...
def run_policy(market, dividends, target, regime, *, policy: str, fin_scores) -> dict:
    need_scores = policy in (FIN_TOP1, FIN_TOP2_EQUAL)
...
def main() -> int:
    ap = argparse.ArgumentParser(description="FIN within-sleeve Stage B (paper only)")
    ...
    for i, policy in enumerate(policies, 1):
        ...
        results[policy] = run_policy(
            market, dividends, target, regime, policy=policy, fin_scores=fin_scores
        )
        r = results[policy]
    ...
    payload = {
        ...
        "label": "FIN_WITHIN_SLEEVE_ALLOC_STAGE_B",
    ...
    RESEARCH.joinpath("FIN_WITHIN_SLEEVE_ALLOC_STAGE_B.json").write_text(
    ...
    lines = [
        ...
        f"- tip positions `{base['tip'].get('tip_fin_positions')}`",
        "",
        "## Challengers vs BASE",
        "",
    ...
    RESEARCH.joinpath("FIN_WITHIN_SLEEVE_ALLOC_STAGE_B.md").write_text(md)
    print(json.dumps({"status": payload["status"], "top": [t["id"] for t in top], "best": best}, indent=2))
    return 0
