#!/usr/bin/env python3
"""Held-out for Stage-8C S8C1 = SLEEVE_VAL_CASH085 × COMBO_VOL80_VAL05 (EXPERIMENTAL).

Locked at OOF:
  detector: not crisis_vote2
            AND rolling-252d mkt_vol_60d >= p80 of prior window (hysteresis 2/5)
            AND val_ic_lag21 >= 0.05
  controller: switch to VALUE sleeve on flag; scale exposure 0.85 on those days
  bull: TECH2 + C4

NOT an in-place E45 edit. No retune after held-out.
"""
...
from e50a3r1_stage7_crisis_challenger_oof import attach_crisis, period_metrics, merge_orders_crisis_sleeve
...
S8C1 = {
    "challenger_id": "S8C1",
    "controller": "SLEEVE_VAL_CASH085",
    "detector": "COMBO_VOL80_VAL05",
    "vol_roll_window": 252,
    "vol_roll_pctl": 0.80,
    "val_ic_min": 0.05,
    "cash_scale": 0.85,
...
def build_value_scores_range(panel: pl.DataFrame, start: date, end: date) -> pl.DataFrame:
    d = panel.filter(pl.col("date").is_between(start, end)).select(
...
def build_flags(panel, labels, execution) -> dict[date, bool]:
    ...
    val_ok = np.array([
        (r.get("val_ic_lag21") is not None and r["val_ic_lag21"] >= S8C1["val_ic_min"])
        for r in rows
    ])
    vol_raw = rolling_percentile_flags(vol, S8C1["vol_roll_window"], S8C1["vol_roll_pctl"])
    vol_h = hysteresis(vol_raw, S8C1["hysteresis_on"], S8C1["hysteresis_off"])
    flags_arr = (~crisis) & vol_h & val_ok
    return {d: bool(flags_arr[i]) for i, d in enumerate(dates)}


def evaluate_period(
    joined, panel, execution, calendar, flags, cash_scale, name, start, end, fit_cutoff, stress_dates,
):
    ...
    val_scores = build_value_scores_range(panel, start, end)
    val_orders, _ = buffered_orders_ext(val_scores, calendar, **C4)
    switched = merge_orders_crisis_sleeve(tech_orders, val_orders, flags)
    if cash_scale < 1.0:
        orders = scale_orders(
            switched,
        ...
        orders = tech_orders  # BASE full bull sleeve
    ...
    period_dates = scored["date"].unique().to_list()
    stress_on_days = sum(1 for d in period_dates if flags.get(d, False))
    out = {
        "portfolio": name,
        "controller": "SLEEVE_VAL_CASH085" if cash_scale < 1.0 else "BASE_FULL",
        ...
        "stress_flag_share": stress_on_days / max(len(period_dates), 1),
...
def main() -> None:
    ...
    ap.add_argument("--stage8c-summary", type=Path, required=True)
    ...
    summary = json.loads(args.stage8c_summary.read_text())
    if summary["research_decision"] != "OOF_NEW_MULTISLEEVE_DUAL_GATE_WINNER":
        raise RuntimeError("Stage-8C did not produce a multi-sleeve dual-gate winner")
    rec = summary["recommended"]
    if rec["controller"] != "SLEEVE_VAL_CASH085" or rec["detector"] != "COMBO_VOL80_VAL05":
        raise RuntimeError(f"S8C1 lock drift: {rec['controller']} / {rec['detector']}")
    ...
    print("building COMBO_VOL80_VAL05 flags (rolling p80 + val_ic>=0.05) ...", flush=True)
    ...
    print("S8C1 validation ...", flush=True)
    val, val_nav, val_trades, val_proxy = evaluate_period(
        joined, panel, execution, calendar, flags, S8C1["cash_scale"], "S8C1_VALIDATION_2019_2022",
        validation_start, validation_end, val_cutoff, val_stress,
    )
    print("S8C1 sealed ...", flush=True)
    sealed, sealed_nav, sealed_trades, sealed_proxy = evaluate_period(
        joined, panel, execution, calendar, flags, S8C1["cash_scale"], "S8C1_SEALED_2023_LATEST",
        sealed_start, sealed_end, sealed_cutoff, sealed_stress,
    )
    print("C4 full bull reference validation/sealed ...", flush=True)
    c4_val, _, _, _ = evaluate_period(
        joined, panel, execution, calendar, flags, 1.0, "C4_FULL_VALIDATION_2019_2022",
        validation_start, validation_end, val_cutoff, val_stress,
    )
    c4_sealed, _, _, _ = evaluate_period(
        joined, panel, execution, calendar, flags, 1.0, "C4_FULL_SEALED_2023_LATEST",
        sealed_start, sealed_end, sealed_cutoff, sealed_stress,
    )

    label = classify(val, sealed)
    for tag, nav, trades, proxy in [
        ("s8c1_validation_2019_2022", val_nav, val_trades, val_proxy),
        ("s8c1_sealed_2023_latest", sealed_nav, sealed_trades, sealed_proxy),
    ]:
    ...
    decision = {
        ...
        "stress_vs_c4_validation": {
            "s8c1_stress_ex": val.get("s_crisis_mean_excess"),
            "c4_stress_ex": c4_val.get("s_crisis_mean_excess"),
            "s8c1_stress_comp": val.get("s_crisis_strategy_compound"),
    ...
    (out / "reports" / "stage8c_s8c1_heldout_decision.json").write_text(
        json.dumps(decision, indent=2, default=str) + "\n"
    )
    lines = [
        "# E50-A3-R1-S8C1 SLEEVE_VAL_CASH085 × COMBO_VOL80_VAL05 — Held-Out",
        "",
        "Locked OOF multi-sleeve: VALUE sleeve + 0.85 cash when rolling-vol p80 & val IC≥0.05 & non-crisis. "
        "**No retune. Not an in-place E45 edit.**",
        "",
        f"## Research decision",
        "",
        f"**`{label}`**",
        "",
        f"OOF: util={rec['utility']:.4f}, boot={rec['block_bootstrap_positive_probability']}, "
        f"stress_ex={rec['s_crisis_mean_excess']}",
        "",
        "| Metric | S8C1 Val | S8C1 Sealed | C4 Full Val | C4 Full Sealed |",
        "|---|---:|---:|---:|---:|",
        ...
        "",
        "Artifact: `reports/stage8c_s8c1_heldout_decision.json`",
        "",
    ]
    (out / "E50-A3-R1_STAGE8C_S8C1_HELDOUT.md").write_text("\n".join(lines))
    print(json.dumps({
        "research_decision": label,
        ...
        "val_stress_share": val["stress_flag_share"],
