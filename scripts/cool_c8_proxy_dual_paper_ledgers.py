#!/usr/bin/env python3
"""COOL_c8_f50_d21 dual-paper ledgers — OPERATING OBSERVE (paper only).

LIVE_STACK ∥ COOL_c8_f50_d21 (PROXY circuit + cool=8). Soft-Frozen KEEP · no live wire.
"""
...
OUT = ROOT / "repro/cool-c8-proxy-dual-paper-observe"
...
def post_base(ctx: PostBaseCtx) -> None:
    ...
    exposure = build_cool_c8_exposure(dates, feat["proxy_mdd63"])
    ...
    ctx.prepared.extras["cool_c8_f50_d21_exposure.csv"] = exposure.rename("e45_exposure")
...
def report(result: LedgerResult) -> None:
    ...
    payload = {
        "schema_version": "cool_c8_proxy_dual_paper_observe_v1",
        "label": "COOL_C8_PROXY_DUAL_PAPER_OBSERVE_OPERATING",
        ...
        "non_actions": [
            "No Soft-Frozen / Soft / Sleeve / FUSE / DH live flip from this observe",
    ...
    body_lines += ["", "Repro: `repro/cool-c8-proxy-dual-paper-observe/`", ""]

    write_json_md_pair(
        out_dir=OUT,
        report_stem="COOL_C8_PROXY_DUAL_PAPER_OBSERVE_OPERATING",
        payload=payload,
        md_lines=body_lines,
        mirror_dirs=(OPS,),
    )
    ...
    (OPS / "COOL_C8_PROXY_DUAL_PAPER_OBSERVE.json").write_text(text, encoding="utf-8")
...
SPEC = DualPaperLedgerSpec(
    ...
    chal_nav_name="cool_c8_f50_d21_daily_nav.csv",
    write_fills=False,
