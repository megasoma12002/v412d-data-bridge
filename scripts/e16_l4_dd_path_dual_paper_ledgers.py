#!/usr/bin/env python3
"""L4_DD_PATH_08_50 dual-paper ledgers — thin wrapper over ops_dual_paper_ledgers."""
...
from mdd_l4_loss_engine_adv_lite import dd_path_target
...
def prepare(market, dividends) -> PreparedBooks:
    ...
    flag_df = pd.DataFrame({"date": l4_flag.index, "dd_path_cap_on": l4_flag.astype(bool).values})
    return PreparedBooks(
        base_target=base_target,
        base_regime=base_regime,
        chal_target=l4_target,
        chal_regime=base_regime,
        extras={"l4_dd_path_cap_flag.csv": flag_df},
        context={"dd_thr": LOCKED_DD_THR},
    )
...
def report(result: LedgerResult) -> None:
    ...
    payload = {
        ...
        "label": "L4_DD_PATH_DUAL_PAPER_LEDGERS",
    ...
    lines = [
        "# L4_DD_PATH Dual-Paper Ledgers",
        "",
        f"Generated: `{payload['generated_at_utc']}`",
        f"Locked: **{LOCKED_ID}** (dd_thr={LOCKED_DD_THR})",
        f"Held-out score **{held.get('score')}** · Sealed **{sealed.get('score')}**",
        "",
    ]
    write_json_md_pair(
        out_dir=OUT,
        report_stem="l4_dd_path_dual_paper",
        payload=payload,
        md_lines=lines,
        mirror_dirs=(GAPS,),
        mirror_stem="L4_DD_PATH_DUAL_PAPER",
    )
...
SPEC = DualPaperLedgerSpec(
    ...
    chal_nav_name="l4_dd_path_daily_nav.csv",
    compare_chal_col="nav_l4",
    compare_rel_col="rel_l4_vs_base",
    base_targets_name="base_e16_targets.csv",
    chal_targets_name="l4_dd_path_targets.csv",
