#!/usr/bin/env python3
"""E45 defend→handoff dual-paper month-end monitor — OPERATING OBSERVE.

Compares LIVE_STACK vs DH_dd06_vz1p0.
Paper-only; Soft-Frozen KEEP; no live wire; no E45 stitch.
"""
...
from e45_defend_handoff_helpers import BASE_ID, CHAL_ID as FUSE_ID
...
CHAL_NAV = (
    ROOT / "repro/e45-defend-handoff-dual-paper-observe/outputs/dh_dd06_vz1p0_daily_nav.csv"
)
COMPARE_NAV = ROOT / "repro/e45-defend-handoff-dual-paper-observe/outputs/dual_paper_nav_compare.csv"
...
def _load_books() -> tuple[pd.DataFrame, pd.DataFrame, str]:
    ...
    if COMPARE_NAV.exists():
        ...
        if not {"nav_base", "nav_chal"}.issubset(d.columns):
            raise SystemExit(
                ...
                "run scripts/e16_e45_defend_handoff_dual_paper_ledgers.py"
            )
    ...
    raise SystemExit(
        "Missing E45_DEFEND_HANDOFF observe NAV. Run scripts/e16_e45_defend_handoff_dual_paper_ledgers.py"
    )
...
def _alerts(rows: list[dict]) -> list[str]:
    ...
    for wname in ("ytd", "trailing_1y"):
        ...
        if r["mdd_improve_pp"] is not None and r["mdd_improve_pp"] < 0:
            out.append(f"ALERT: {FUSE_ID} {wname} MDD worse than {BASE_ID}")
        gb = r["cagr_giveback_pp"]
        if gb is None:
            continue
        if gb > TRAIL_ALERT_PP:
            out.append(f"ALERT: {FUSE_ID} {wname} CAGR giveback > {TRAIL_ALERT_PP:.1f} pp")
        if gb > TRAIL_PAUSE_PP:
            out.append(f"PAUSE_REVIEW: {FUSE_ID} {wname} giveback > {TRAIL_PAUSE_PP:.0f} pp")
    for wname in ("heldout_2019_plus", "sealed_2023_plus"):
        ...
        if gb is not None and gb > tgt + STRUCTURAL_BUFFER_PP:
            out.append(
                f"ALERT: {FUSE_ID} {wname} giveback {gb:.2f} > design {tgt:.2f}+{STRUCTURAL_BUFFER_PP:.0f}"
            )
    return out
...
def main() -> int:
    ...
    payload = {
        ...
        "e45_stitch": "FORBIDDEN",
        "independent_observes": "Soft/Sleeve/FUSE KEEP",
        ...
        "non_actions": [
            ...
            "Soft/Sleeve/FUSE observes KEEP",
            "no undo DROP_E45_A05",
        ],
    }
    (args.out / "month_end_monitor.json").write_text(
        json.dumps(payload, indent=2, default=str) + "\n", encoding="utf-8"
    )
    OPS.joinpath("E45_DEFEND_HANDOFF_MONTH_END_MONITOR.json").write_text(
        json.dumps(payload, indent=2, default=str) + "\n", encoding="utf-8"
    )
    lines = [
        f"# E45_DEFEND_HANDOFF month-end monitor (asof {asof.date()})",
        "",
        f"Status: `{STATUS}` · paper only · base `{BASE_ID}` vs `{FUSE_ID}`",
        "",
    ...
    OPS.joinpath("E45_DEFEND_HANDOFF_MONTH_END_MONITOR.md").write_text(md, encoding="utf-8")
