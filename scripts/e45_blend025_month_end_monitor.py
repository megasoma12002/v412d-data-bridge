#!/usr/bin/env python3
"""E45 blend-α=0.25 month-end dual-paper monitor (ops observe — not live).

Compares BASE_E16_E18_E22_v2s vs BLEND_E45_A25 paper NAVs at month-end (or as-of).
Does NOT change Soft-Frozen clips. Does NOT place orders. Stitch always blocked.

Inputs (from dual-paper harness):
  repro/e45-blend025-dual-paper-observe/outputs/base_e16_e18_e22_v2s_daily_nav.csv
  repro/e45-blend025-dual-paper-observe/outputs/blend_e45_a25_daily_nav.csv

Optional: re-run `scripts/e45_blend025_dual_paper_ledgers.py` first to refresh.
"""
...
LOCKED_ID = "BLEND_E45_A25"
# Dynamic trailing gates (YTD / 1y).
TRAIL_ALERT_PP = 3.0
TRAIL_PAUSE_PP = 5.0
# Design-known structural givebacks (Stage-3 design pack, 2026-09-05).
DESIGN_GIVEBACK_PP = {
    # From E45_BLEND_ALPHA_PAPER_SCREEN (α=0.25, 2026-09-06)
    "heldout_2019_plus": 2.83,
    "sealed_2023_plus": 4.36,
}
...
def main() -> None:
    ...
    if not BASE_NAV.exists() or not CHAL_NAV.exists():
        raise SystemExit(
            "Missing dual-paper NAVs. Run scripts/e45_blend025_dual_paper_ledgers.py first."
        )
        ...
        rows.append(
            {
                ...
                "blend_e45_a25_cagr": sc["cagr"],
                "blend_e45_a25_mdd": sc["max_drawdown"],
    ...
    summary = {
        ...
        "note": (
            ...
            "E45 blend-α=0.25 dual-paper ≠ stitch license. "
    ...
    lines = [
        f"# E45 Blend-α=0.25 Month-End Paper Monitor — asof {asof.date()}",
        "",
        f"Generated: `{summary['generated_at_utc']}`",
        "Status: **OPERATING OBSERVE / PAPER ONLY** — Soft-Frozen live default unchanged.",
        f"Locked: **{LOCKED_ID}** (α=0.25 × E45 `E3_VOLTARGET_WINNER` on early-stack)",
        "",
        ...
        "",
        "| Window | BASE CAGR | BASE MDD | BLEND_E45_A25 CAGR | BLEND_E45_A25 MDD | "
    ...
    for r in rows:
        ...
        chal_cagr_note = _pct(r["blend_e45_a25_cagr"])
        if r["window"] == "mtd":
        ...
        lines.append(
            ...
            f"{chal_cagr_note} | {_pct(r['blend_e45_a25_mdd'])} | "
    ...
    (RESEARCH / "E45_BLEND025_MONTH_END_MONITOR.md").write_text(md)
    (RESEARCH / "E45_BLEND025_MONTH_END_MONITOR.json").write_text(json.dumps(summary, indent=2) + "\n")
    print(
        json.dumps(
            {
                "asof": summary["asof"],
