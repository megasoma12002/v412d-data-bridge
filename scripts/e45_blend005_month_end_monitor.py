#!/usr/bin/env python3
"""E45 blend-α=0.05 month-end dual-paper monitor (ops observe — not live).

Compares BASE_E16_E18_E22_v2s vs BLEND_E45_A05 paper NAVs at month-end (or as-of).
Does NOT change Soft-Frozen clips. Does NOT place orders. Stitch always blocked.

Inputs (from dual-paper harness):
  repro/e45-blend005-dual-paper-observe/outputs/base_e16_e18_e22_v2s_daily_nav.csv
  repro/e45-blend005-dual-paper-observe/outputs/blend_e45_a05_daily_nav.csv

Optional: re-run `scripts/e45_blend005_dual_paper_ledgers.py` first to refresh.
"""
...
DESIGN_GIVEBACK_PP = {
    # From E45_LOW_ALPHA_DEEP_DIVE / P1 (α=0.05, 2026-09-06)
    "heldout_2019_plus": 1.07,
    "sealed_2023_plus": 1.50,
}
...
def main() -> None:
        ...
        rows.append(
            {
                ...
                "blend_e45_a05_cagr": sc["cagr"],
                "blend_e45_a05_mdd": sc["max_drawdown"],
    ...
    summary = {
        ...
        "note": (
            ...
            "E45 blend-α=0.05 dual-paper ≠ stitch license. "
    ...
    lines = [
        f"# E45 Blend-α=0.05 Month-End Paper Monitor — asof {asof.date()}",
        "",
        f"Generated: `{summary['generated_at_utc']}`",
        "Status: **OPERATING OBSERVE / PAPER ONLY** — Soft-Frozen live default unchanged.",
        f"Locked: **{LOCKED_ID}** (α=0.05 × E45 `E3_VOLTARGET_WINNER` on early-stack)",
        "",
        ...
        "",
        "| Window | BASE CAGR | BASE MDD | BLEND_E45_A05 CAGR | BLEND_E45_A05 MDD | "
    ...
    for r in rows:
        ...
        chal_cagr_note = _pct(r["blend_e45_a05_cagr"])
        if r["window"] == "mtd":
        ...
        lines.append(
            ...
            f"{chal_cagr_note} | {_pct(r['blend_e45_a05_mdd'])} | "
    ...
    (RESEARCH / "E45_BLEND005_MONTH_END_MONITOR.md").write_text(md)
    (RESEARCH / "E45_BLEND005_MONTH_END_MONITOR.json").write_text(json.dumps(summary, indent=2) + "\n")
    print(
        json.dumps(
            {
                "asof": summary["asof"],
