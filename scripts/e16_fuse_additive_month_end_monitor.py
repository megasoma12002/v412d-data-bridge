#!/usr/bin/env python3
"""FUSE_ADDITIVE dual-paper month-end monitor — OPERATING OBSERVE.

Compares LIVE_STACK vs FUSE_ADDITIVE.
Paper-only; Soft-Frozen clips KEEP; no live wire; Soft/Sleeve observes KEEP independent.
"""
...
CHAL_NAV = (
    ROOT / "repro/fuse-additive-dual-paper-observe/outputs/fuse_additive_daily_nav.csv"
)
...
def _load_books() -> tuple[pd.DataFrame, pd.DataFrame, str]:
    ...
    if COMPARE_NAV.exists():
        ...
        if not {"nav_base", "nav_chal"}.issubset(d.columns):
            raise SystemExit(
                ...
                "run scripts/e16_fuse_additive_dual_paper_ledgers.py"
            )
    ...
    raise SystemExit(
        "Missing FUSE_ADDITIVE observe NAV. Run scripts/e16_fuse_additive_dual_paper_ledgers.py"
    )
...
def main() -> int:
    ...
    OPS.joinpath("FUSE_ADDITIVE_MONTH_END_MONITOR.json").write_text(
        json.dumps(payload, indent=2, default=str) + "\n", encoding="utf-8"
    )
    lines = [
        f"# FUSE_ADDITIVE month-end monitor (asof {asof.date()})",
        "",
    ...
    OPS.joinpath("FUSE_ADDITIVE_MONTH_END_MONITOR.md").write_text(md, encoding="utf-8")
