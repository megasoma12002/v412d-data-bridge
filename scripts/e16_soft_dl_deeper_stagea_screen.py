#!/usr/bin/env python3
"""Stage A paper: Soft-assist deeper/wider MLP (numpy) buy soft — NO live wire.

Charter: research/ops/SOFT_DL_DEEPER_STAGEA_CHARTER.md

Follow-on to tiny-MLP Stage A. Walk-forward wider (h16) and deeper (16→8→1)
numpy nets on causal TA LOW features → additive Soft buy boost on LIVE KD_OPT.
Soft-Frozen KEEP · TEL_EQUAL KEEP · Soft-assist observe KEEP · Sleeve observe
UNCHANGED · no Soft×Sleeve fuse · E45 OFF.
"""
...
OUT = ROOT / "repro/soft-dl-deeper-stagea"
...
class DeepMLP:
    ...
    def __init__(self, n_in: int, h1: int = 16, h2: int = 8, seed: int = SEED):
        rng = np.random.default_rng(seed)
        self.W1 = rng.normal(0.0, 0.25, size=(n_in, h1))
        self.b1 = np.zeros(h1)
        self.W2 = rng.normal(0.0, 0.25, size=(h1, h2))
        self.b2 = np.zeros(h2)
        self.W3 = rng.normal(0.0, 0.25, size=(h2, 1))
        self.b3 = np.zeros(1)

    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        h1 = np.tanh(X @ self.W1 + self.b1)
        h2 = np.tanh(h1 @ self.W2 + self.b2)
        return _sigmoid(h2 @ self.W3 + self.b3).ravel()
        ...
        *,
        ...
        for _ in range(epochs):
            ...
            for s in range(0, n, batch):
                b = idx[s : s + batch]
                xb, yb = X[b], y[b]
                h1 = np.tanh(xb @ self.W1 + self.b1)
                h2 = np.tanh(h1 @ self.W2 + self.b2)
                p = _sigmoid(h2 @ self.W3 + self.b3)
                dlogit = (p - yb) / max(len(b), 1)
                dW3 = h2.T @ dlogit + l2 * self.W3
                db3 = dlogit.sum(axis=0)
                dh2 = dlogit @ self.W3.T * (1.0 - h2 ** 2)
                dW2 = h1.T @ dh2 + l2 * self.W2
                db2 = dh2.sum(axis=0)
                dh1 = dh2 @ self.W2.T * (1.0 - h1 ** 2)
                dW1 = xb.T @ dh1 + l2 * self.W1
                db1 = dh1.sum(axis=0)
                self.W3 -= lr * dW3
                self.b3 -= lr * db3
                self.W2 -= lr * dW2
...
def walk_forward_soft_panel(
    dates, codes, X, fwd, *, kind: str, alpha: float, n_hidden: int = 8
) -> pd.DataFrame:
    ...
    for y in years:
        ...
        if not apply_mask.any():
            continue
        Xt_raw = X[train_mask].reshape(-1, X.shape[-1]).astype(np.float64)
        yt_raw = fwd[train_mask].reshape(-1)
        ok = np.isfinite(yt_raw)
        if int(ok.sum()) < MIN_TRAIN_ROWS:
            continue
        Xt = Xt_raw[ok]
        yt = (yt_raw[ok] > 0.0).astype(np.float64)
        n_in = Xt.shape[1]
        if kind == "mlp":
            model = TinyMLP(n_in, n_hidden=int(n_hidden), seed=SEED + y)
            model.fit(Xt, yt)
        elif kind == "deep":
            model = DeepMLP(n_in, h1=16, h2=8, seed=SEED + y)
            model.fit(Xt, yt)
        else:
...
def main() -> int:
    ...
    print("building walk-forward Soft panels (h8 / h16 / deep) ...", flush=True)
    dates, X = build_feature_cube(lows, list(FIN))
    fwd = build_fwd_returns(market, list(FIN), dates)
    soft_h8_a05 = walk_forward_soft_panel(dates, list(FIN), X, fwd, kind="mlp", alpha=0.5, n_hidden=8)
    soft_h16_a05 = walk_forward_soft_panel(dates, list(FIN), X, fwd, kind="mlp", alpha=0.5, n_hidden=16)
    soft_deep_a05 = walk_forward_soft_panel(dates, list(FIN), X, fwd, kind="deep", alpha=0.5)
    soft_deep_a025 = walk_forward_soft_panel(dates, list(FIN), X, fwd, kind="deep", alpha=0.25)
    ...
    soft_h8_a05 = align_boost(soft_h8_a05)
    soft_h16_a05 = align_boost(soft_h16_a05)
    soft_deep_a05 = align_boost(soft_deep_a05)
    soft_deep_a025 = align_boost(soft_deep_a025)

    jobs = [
        (SOFT_BASE, "base", None, False),
        (OBSERVE_CHAL_ID, "observe", add_buy_softs(kd_scores, lows, [(BUY_LOW_ID, 1.0), ("K9_LT30", 1.0)]), True),
        ("DL_SOFT_MLP_h8_a05", "dl_mlp_h8", kd_scores.astype(float) + soft_h8_a05, True),
        ("DL_SOFT_MLP_h16_a05", "dl_mlp_h16", kd_scores.astype(float) + soft_h16_a05, True),
        ("DL_SOFT_MLP_d16_8_a05", "dl_mlp_deep", kd_scores.astype(float) + soft_deep_a05, True),
        ("DL_SOFT_MLP_d16_8_a025", "dl_mlp_deep", kd_scores.astype(float) + soft_deep_a025, True),
    ]
    ...
    payload = {
        ...
        "label": "SOFT_DL_DEEPER_STAGEA_SCREEN",
        "charter": "research/ops/SOFT_DL_DEEPER_STAGEA_CHARTER.md",
        "status": "PAPER_SCREEN_DONE",
        "live_wire": False,
        "soft_x_sleeve_fuse": False,
        "model": {
            "type": "wider_h16_and_deep_16_8_plus_h8_baseline",
    ...
    (OUT / "reports" / "soft_dl_deeper_stagea_screen.json").write_text(blob, encoding="utf-8")
    (OPS / "SOFT_DL_DEEPER_STAGEA_SCREEN.json").write_text(blob, encoding="utf-8")
    ...
    lines = [
        "# Soft-Assist Deeper-MLP Stage A Screen",
        "",
        f"Generated: `{payload['generated_at_utc']}` · asof **{payload['asof']}**",
        f"Verdict: **`{verdict}`** · books **{payload['n_books']}** · framework **numpy deeper/wider MLP**",
        "Live wire: **false** · Soft×Sleeve fuse: **forbidden**",
        "",
        "## Question",
        "",
        "Can a walk-forward deeper/wider MLP (or linear ablation) additive Soft buy score tip-clean / "
    ...
    lines += [
        ...
        "## Reading",
        "",
        "- Shallow baseline = `feat→8→1` α=0.5; wider = `feat→16→1`; deeper = `feat→16→8→1`.",
        "- Features = causal TA LOW bools; train years `< Y`, apply year `Y`.",
        "- Sell soft kept as observe `RSI6_GT80` to isolate buy-side DL.",
        "- Deeper/wider numpy MLP follow-on — **still** not live-ready deep learning.",
        "",
        "## Non-actions",
        "",
    ]
    lines.extend(f"- {x}" for x in payload["non_actions"])
    lines += ["", "## Label", "", f"`SOFT_DL_DEEPER_STAGEA_SCREEN_{payload['asof']}__{verdict}`", ""]
    md = "\n".join(lines)
    (OUT / "reports" / "SOFT_DL_DEEPER_STAGEA_SCREEN.md").write_text(md, encoding="utf-8")
    (OPS / "SOFT_DL_DEEPER_STAGEA_SCREEN.md").write_text(md, encoding="utf-8")

    zh = "\n".join([
        "# Soft-Assist Deeper-MLP Stage A 篩選",
        "",
        f"產生：`{payload['generated_at_utc']}` · asof **{payload['asof']}**",
        f"判決：**`{verdict}`** · books **{payload['n_books']}** · **numpy 更深 MLP**",
        ...
        "",
        "詳表見英文 `SOFT_DL_DEEPER_STAGEA_SCREEN.md`。",
        "",
        "## 非動作",
        "",
        ...
        "- 不 Soft×Sleeve 融合",
        "",
        f"`SOFT_DL_DEEPER_STAGEA_SCREEN_{payload['asof']}__{verdict}`",
        "",
    ])
    (OPS / "SOFT_DL_DEEPER_STAGEA_SCREEN.zh-TW.md").write_text(zh, encoding="utf-8")

    charter = OPS / "SOFT_DL_DEEPER_STAGEA_CHARTER.md"
    if charter.exists():
        ct = charter.read_text()
        ct = ct.replace("**PAPER STAGE A OPEN**", f"**PAPER STAGE A DONE** · verdict **`{verdict}`**")
        ct = ct.replace(
            "`SOFT_DL_DEEPER_STAGEA_CHARTER_2026-09-12__PAPER_OPEN`",
            f"`SOFT_DL_DEEPER_STAGEA_CHARTER_2026-09-12__{verdict}`",
        )
