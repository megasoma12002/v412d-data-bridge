#!/usr/bin/env python3
"""Held-out for Stage-9A S9A1 = FREEZE_REB × COMBO_VOL70_VAL03 (EXPERIMENTAL / E45-C1).

Locked at OOF:
  detector: not crisis_vote2
            AND rolling-252d mkt_vol_60d >= p70 (hysteresis 2/5)
            AND val_ic_lag21 >= 0.03
  controller: skip C4 rebalance dates while detector on (hold names)
  bull: TECH2 + C4
...
def build_flags(panel, labels, execution) -> dict[date, bool]:
    ...
    val_ok = np.array([
        (r.get("val_ic_lag21") is not None and r["val_ic_lag21"] >= S9A1["val_ic_min"])
        for r in rows
    ])
...
def evaluate_period(joined, execution, calendar, flags, freeze: bool, name, start, end, fit_cutoff, stress_dates):
    model = r1.fit_model(joined, "TECH2", "BREADTH_REGIME", 1.0, fit_cutoff)
    scored = r1.score_period(joined, model, start, end)
    if freeze:
        sig_dates = freeze_signal_dates(scored, flags, C4["rebalance_every"])
        orders = orders_on_dates(scored, calendar, sig_dates, C4)
    else:
    ...
    out = {
        "portfolio": name,
        "controller": "FREEZE_REB" if freeze else "BASE_FULL",
...
def main() -> None:
    ...
    ap.add_argument("--stage9a-summary", type=Path, required=True)
    ...
    summary = json.loads(args.stage9a_summary.read_text())
    if summary["research_decision"] != "OOF_NEW_E45C1_CONTROLLER_DUAL_GATE_WINNER":
        raise RuntimeError("Stage-9A did not produce an E45-C1 dual-gate winner")
    rec = summary["recommended"]
    if rec["controller"] != "FREEZE_REB" or rec["detector"] != "COMBO_VOL70_VAL03":
        raise RuntimeError(f"S9A1 lock drift: {rec['controller']} / {rec['detector']}")
    ...
    print("building COMBO_VOL70_VAL03 flags ...", flush=True)
    ...
    print("S9A1 validation ...", flush=True)
    val, val_nav, val_trades, val_proxy = evaluate_period(
        joined, execution, calendar, flags, True, "S9A1_VALIDATION_2019_2022",
        validation_start, validation_end, val_cutoff, val_stress,
    )
    print("S9A1 sealed ...", flush=True)
    sealed, sealed_nav, sealed_trades, sealed_proxy = evaluate_period(
        joined, execution, calendar, flags, True, "S9A1_SEALED_2023_LATEST",
        sealed_start, sealed_end, sealed_cutoff, sealed_stress,
    )
    print("C4 full reference ...", flush=True)
    c4_val, _, _, _ = evaluate_period(
        joined, execution, calendar, flags, False, "C4_FULL_VALIDATION_2019_2022",
        validation_start, validation_end, val_cutoff, val_stress,
    )
    c4_sealed, _, _, _ = evaluate_period(
        joined, execution, calendar, flags, False, "C4_FULL_SEALED_2023_LATEST",
        sealed_start, sealed_end, sealed_cutoff, sealed_stress,
    )

    label = classify(val, sealed)
    for tag, nav, trades, proxy in [
        ("s9a1_validation_2019_2022", val_nav, val_trades, val_proxy),
        ("s9a1_sealed_2023_latest", sealed_nav, sealed_trades, sealed_proxy),
    ]:
    ...
    decision = {
        ...
        "stress_vs_c4_validation": {
            "s9a1_stress_ex": val.get("s_crisis_mean_excess"),
            "c4_stress_ex": c4_val.get("s_crisis_mean_excess"),
            "s9a1_stress_comp": val.get("s_crisis_strategy_compound"),
    ...
    (out / "reports" / "stage9a_s9a1_heldout_decision.json").write_text(
        json.dumps(decision, indent=2, default=str) + "\n"
    )
    lines = [
        "# E50-A3-R1-S9A1 FREEZE_REB × COMBO_VOL70_VAL03 — Held-Out (E45-C1)",
        "",
        "Locked OOF: skip rebalance while rolling-vol p70 & val IC≥0.03 & non-crisis. "
    ...
    (out / "E50-A3-R1_STAGE9A_S9A1_HELDOUT.md").write_text("\n".join(lines))
    print(json.dumps({
        "research_decision": label,
