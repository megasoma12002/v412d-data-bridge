...
Alphas: 0.00 (BASE), 0.25, 0.35, 0.50, 1.00 (full CHAL_E45_E3).
...
OUT = ROOT / "repro/e45-blend-alpha-screen"
...
def main() -> None:
    ...
    summary = {
        ...
        "label": "E45_BLEND_ALPHA_PAPER_SCREEN",
        "status": "PAPER_ONLY",
        "live_wire": False,
    ...
    (OUT / "reports" / "e45_blend_alpha_screen.json").write_text(
        json.dumps(summary, indent=2) + "\n"
    )
    (RESEARCH / "E45_BLEND_ALPHA_PAPER_SCREEN.json").write_text(
        json.dumps(summary, indent=2) + "\n"
    )
    ...
    lines += [
        ...
        "",
        "Artifacts:",
        f"- `{OUT / 'reports' / 'e45_blend_alpha_screen.json'}`",
        f"- `{OUT / 'outputs' / 'blend_alpha_window_metrics.csv'}`",
        f"- `{OUT / 'outputs' / 'blend_alpha_deltas_vs_base.csv'}`",
        "",
    ]
    md = "\n".join(lines)
    (OUT / "E45_BLEND_ALPHA_PAPER_SCREEN.md").write_text(md)
    (RESEARCH / "E45_BLEND_ALPHA_PAPER_SCREEN.md").write_text(md)
    (OPS / "E45_BLEND_ALPHA_PAPER_SCREEN.md").write_text(
        "# E45 Blend-Alpha Paper Screen (pointer)\n\n"
        "Status: **PAPER ONLY** — see `research/e45/E45_BLEND_ALPHA_PAPER_SCREEN.md`.\n\n"
        "Run:\n\n"
        "```bash\n"
        "python3 scripts/e45_blend_alpha_paper_screen.py\n"
        "```\n\n"
