...
| Broker live-write 防呆 | `broker_safety` + **`broker_risk`**（狀態機／panic／黑名單／限額／限流／重啟對帳／熔斷讀寫分離／告警）· `BROKER_RISK_RECOVERY.md`；`LIVE.broker_live_write_accepted=False` until ACCEPT |
