# E45 Non-COVID Multi-Event — Alternate Levers

Generated: `2026-09-06T06:53:45.232253+00:00`
Status: **PAPER ONLY** — Soft-Frozen **KEEP** · DEFAULT **`E22_v2s_tw` KEEP** · stitch **FORBIDDEN**
Claimed −13.16%: **`RETIRED_HISTORICAL_NARRATIVE`** (do not invent a replacement)

## Scope

Beyond already-failed mild ALL / FIN_ONLY / HIGH_BETA grids:
TEL_ONLY densify · FIN+0050 denser α · E1_BINARY overlay · crisis-gated blend.

Help threshold: MDD help > **0.25 pp**.
- `incl_covid`: ≥2 of {2015,2018,2020,2022}
- `strict_noncovid`: ≥2 of {2015,2018,2022}

## Qualifiers

- incl_covid qualifiers: **0**
- strict_noncovid qualifiers: **0**

| Book | Rule | Years helped | N | Held-out score | COVID-ex held-out | Sealed score | Qualifies? |
|---|---|---|---:|---:|---:|---:|:---:|
| `E1BIN_A05` | incl_covid | [] | 0 | -0.02 | -0.11 | +0.12 | NO |
| `E1BIN_A10` | incl_covid | [] | 0 | -0.03 | -0.12 | +0.13 | NO |
| `E1BIN_A25` | incl_covid | [] | 0 | -0.09 | -0.15 | +0.29 | NO |
| `FIN0050_A03` | incl_covid | [2020] | 1 | +0.16 | -1.21 | +1.20 | NO |
| `FIN0050_A07` | incl_covid | [2020] | 1 | +0.23 | -2.29 | +2.23 | NO |
| `FIN0050_A12` | incl_covid | [2020] | 1 | -0.00 | -2.66 | +2.62 | NO |
| `FIN0050_A15` | incl_covid | [2018, 2020] | 2 | -0.20 | -2.68 | +2.70 | NO |
| `GATE085_A10` | incl_covid | [] | 0 | -0.26 | -0.99 | +1.13 | NO |
| `GATE085_A25` | incl_covid | [] | 0 | -0.43 | -1.26 | +1.07 | NO |
| `GATE090_FIN_A10` | incl_covid | [] | 0 | -0.21 | -1.00 | +1.29 | NO |
| `REF_BLEND_A05` | incl_covid | [2020] | 1 | +0.22 | -2.05 | +2.07 | NO |
| `REF_CHAL_E3` | incl_covid | [2015, 2018, 2020, 2022] | 4 | -1.00 | -2.67 | +0.03 | NO |
| `REF_FIN_ONLY_A10` | incl_covid | [2020] | 1 | +0.29 | -2.41 | +2.52 | NO |
| `TEL_ONLY_A05` | incl_covid | [] | 0 | +0.01 | -0.43 | +0.45 | NO |
| `TEL_ONLY_A10` | incl_covid | [] | 0 | +0.10 | -0.78 | +0.82 | NO |
| `TEL_ONLY_A15` | incl_covid | [2020] | 1 | +0.12 | -0.99 | +1.09 | NO |
| `E1BIN_A05` | strict_noncovid | [] | 0 | -0.02 | -0.11 | +0.12 | NO |
| `E1BIN_A10` | strict_noncovid | [] | 0 | -0.03 | -0.12 | +0.13 | NO |
| `E1BIN_A25` | strict_noncovid | [] | 0 | -0.09 | -0.15 | +0.29 | NO |
| `FIN0050_A03` | strict_noncovid | [] | 0 | +0.16 | -1.21 | +1.20 | NO |
| `FIN0050_A07` | strict_noncovid | [] | 0 | +0.23 | -2.29 | +2.23 | NO |
| `FIN0050_A12` | strict_noncovid | [] | 0 | -0.00 | -2.66 | +2.62 | NO |
| `FIN0050_A15` | strict_noncovid | [2018] | 1 | -0.20 | -2.68 | +2.70 | NO |
| `GATE085_A10` | strict_noncovid | [] | 0 | -0.26 | -0.99 | +1.13 | NO |
| `GATE085_A25` | strict_noncovid | [] | 0 | -0.43 | -1.26 | +1.07 | NO |
| `GATE090_FIN_A10` | strict_noncovid | [] | 0 | -0.21 | -1.00 | +1.29 | NO |
| `REF_BLEND_A05` | strict_noncovid | [] | 0 | +0.22 | -2.05 | +2.07 | NO |
| `REF_CHAL_E3` | strict_noncovid | [2015, 2018, 2022] | 3 | -1.00 | -2.67 | +0.03 | NO |
| `REF_FIN_ONLY_A10` | strict_noncovid | [] | 0 | +0.29 | -2.41 | +2.52 | NO |
| `TEL_ONLY_A05` | strict_noncovid | [] | 0 | +0.01 | -0.43 | +0.45 | NO |
| `TEL_ONLY_A10` | strict_noncovid | [] | 0 | +0.10 | -0.78 | +0.82 | NO |
| `TEL_ONLY_A15` | strict_noncovid | [] | 0 | +0.12 | -0.99 | +1.09 | NO |

## Year MDD help (pp vs BASE)

| Book | 2015 | 2018 | 2020 | 2022 |
|---|---:|---:|---:|---:|
| `E1BIN_A05` | +0.00 | +0.00 | +0.00 | -0.09 |
| `E1BIN_A10` | +0.00 | +0.00 | +0.00 | -0.08 |
| `E1BIN_A25` | +0.00 | +0.00 | +0.00 | -0.05 |
| `FIN0050_A03` | -0.06 | -0.04 | +0.40 | -0.93 |
| `FIN0050_A07` | -0.06 | +0.07 | +0.78 | -1.64 |
| `FIN0050_A12` | -0.07 | +0.24 | +0.78 | -1.73 |
| `FIN0050_A15` | -0.05 | +0.32 | +0.70 | -1.62 |
| `GATE085_A10` | -0.06 | -0.01 | +0.09 | -0.57 |
| `GATE085_A25` | -0.10 | -0.01 | +0.13 | -0.61 |
| `GATE090_FIN_A10` | -0.03 | -0.01 | +0.13 | -0.60 |
| `REF_BLEND_A05` | -0.08 | +0.06 | +0.70 | -1.49 |
| `REF_CHAL_E3` | +0.83 | +0.83 | +1.71 | +0.51 |
| `REF_FIN_ONLY_A10` | -0.06 | +0.17 | +0.91 | -1.67 |
| `TEL_ONLY_A05` | -0.04 | -0.03 | +0.10 | -0.33 |
| `TEL_ONLY_A10` | -0.08 | -0.04 | +0.25 | -0.60 |
| `TEL_ONLY_A15` | -0.13 | -0.04 | +0.32 | -0.75 |

## Read

- 2020=COVID mega-DD help remains **expected**; honesty = non-COVID multi-event + COVID-ex held-out.
- No Soft-Frozen / DEFAULT / stitch change. HIGH_BETA stays DRAFT.

Label: `E45_NONCOVID_MULTIEVENT_ALT_2026-09-06__STITCH_FORBIDDEN`
